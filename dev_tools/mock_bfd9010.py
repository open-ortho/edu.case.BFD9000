...
Usage:
    python dev_tools/mock_bfd9010.py
    python dev_tools/mock_bfd9010.py --port 5001
...
SAMPLE_IMAGE_PATH = (
    Path(__file__).parent.parent / "documentation" / "images" / "example0.png"
)
...
class MockBFD9010RequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _load_sample_image_b64(self) -> str:
        if SAMPLE_IMAGE_PATH.exists():
            try:
                return base64.b64encode(SAMPLE_IMAGE_PATH.read_bytes()).decode("ascii")
            except Exception:
                pass
...
def main() -> None:
    ...
    print(
        ...
        f"Sample image: {SAMPLE_IMAGE_PATH} "
        f"({'found' if SAMPLE_IMAGE_PATH.exists() else 'not found — using fallback 1x1 PNG'})\n"
