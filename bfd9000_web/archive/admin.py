...
class CodingAdmin(TimestampedAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('system', 'version', 'code', 'display', 'meaning')
        }),
    )
...
class AddressAdmin(TimestampedAdmin):
    ...
    fieldsets = (
        ...
        ('Address Details', {
            'fields': ('line1', 'line2', 'city', 'district', 'state', 'postal_code', 'country')
        }),
    )
...
class CollectionAdmin(TimestampedAdmin):
    list_display = ('short_name', 'full_name', 'curator', 'institution', 'start_date', 'end_date')
    ...
    fieldsets = (
        ...
        ('Responsible Parties', {
            'fields': ('curator', 'institution', 'address')
        }),
        ('Timeframe', {
...
class SubjectAdmin(TimestampedAdmin):
    list_display = ('humanname_family', 'humanname_given', 'gender', 'birth_date', 'created_at')
...
class EncounterAdmin(TimestampedAdmin):
    list_display = ('subject', 'actual_period_start', 'actual_period_end', 'procedure_code', 'created_at')
    ...
    fieldsets = (
        ...
        ('Timeframe', {
            'fields': ('actual_period_start', 'actual_period_end')
        }),
        ('Medical Details', {
            'fields': ('diagnosis', 'procedure_code', 'procedure_occurrence_age', 'procedure_occurrence_datetime')
        }),
    )
...
class ImagingStudyAdmin(TimestampedAdmin):
    list_display = ('encounter', 'collection', 'record_type', 'scan_datetime', 'scan_operator')
    ...
    fieldsets = (
        (None, {
            'fields': ('encounter', 'collection')
        }),
        ('Study Details', {
            'fields': ('record_type', 'view', 'body_site', 'laterality')
        }),
        ('Scan Information', {
            'fields': ('scan_datetime', 'scan_operator', 'scan_location', 'device', 'endpoint')
        }),
        ('DICOM Fields', {
            'fields': ('instance_uid', 'instance_sop_class', 'instance_number', 'series_uid', 'series_modality'),
            'classes': ('collapse',)
        }),
        ('Identifiers', {
            'fields': ('identifiers',)
